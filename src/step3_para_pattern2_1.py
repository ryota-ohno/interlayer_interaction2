...
from step3_para_vdw_pattern2 import get_c_vec_vdw
from step3_para_vdw_pattern2 import detect_peaks
...
def init_process(args):
    ...
    params_dict_para={'a':7.3,'b':5.9,'R3':0.0,'R4':0.0,'theta':25.5}##########ここは入力or読み取り
    get_init_para_csv(auto_dir,monomer_name,params_dict_para)
    
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','R3','R4','E','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_ip5','E_ip6','E_ip7','E_ip8','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
    else:
...
def listen(args):
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=14:##計算する層状の分子数
            continue
        else:##エネルギーの内訳全般
            len_queue-=1;machine_type_list.remove(machine_type)
            Ei01=float(E_list[0]);Eip1=float(E_list[1]);Eip2=float(E_list[2]);Eip3=float(E_list[3]);Eip4=float(E_list[4]);Eit1=float(E_list[5]);Eit2=float(E_list[6]);Eit3=float(E_list[7]);Eit4=float(E_list[8]);Ei02=float(E_list[9]);Eip5=float(E_list[10]);Eip6=float(E_list[11]);Eip7=float(E_list[12]);Eip8=float(E_list[13])##ここも計算する分子数に合わせて調整
            E = (Ei01+Eip1+Eip2+Eip3+Eip4+Ei02+Eip5+Eip6+Eip7+Eip8)/2+Eit1+Eit2+Eit3+Eit4
            #### TODO
            df_E.loc[idx, ['E_i01','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','E_i02','E_ip5','E_ip6','E_ip7','E_ip8','E','status']] = [Ei01,Eip1,Eip2,Eip3,Eip4,Eit1,Eit2,Eit3,Eit4,Ei02,Eip5,Eip6,Eip7,Eip8,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_i01':0.,'E_ip1':0.,'E_ip2':0.,'E_ip3':0.,'E_ip4':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'E_i02':0.,'E_ip5':0.,'E_ip6':0.,'E_ip7':0.,'E_ip8':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    while True:
        E_list=[];cxyz_list=[]
        for cx in [cx_init_prev-0.2,cx_init_prev,cx_init_prev+0.2]:
