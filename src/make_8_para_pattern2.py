##pattern2 slipped-parallelを二つ考える
import os
import numpy as np
import pandas as pd
import subprocess
...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    ...
    df_mono=pd.read_csv('~/Working/interlayer_interaction2/{}/assets/monomer.csv'.format(monomer_name))
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,isInterlayer):
    ...
    monomer_array_p1 = get_monomer_xyzR(monomer_name,0,b_,R3,0,0,A3, phi1)##p1がb方向
    monomer_array_ip1 = get_monomer_xyzR(monomer_name,c[0],c[1]+b_,c[2]+R3,0,0,A3, phi1)
    monomer_array_ip2 = get_monomer_xyzR(monomer_name,c[0],c[1]-b_,c[2]-R3,0,0,A3, phi1)

    monomer_array_p2 = get_monomer_xyzR(monomer_name,a_,0,2*R4-R3,0,0,A3, phi1)##p2がa方向
    monomer_array_ip3 = get_monomer_xyzR(monomer_name,c[0]+a_,c[1],c[2]+2*R4-R3,0,0,A3, phi1)
    monomer_array_ip4 = get_monomer_xyzR(monomer_name,c[0]-a_,c[1],c[2]-(2*R4-R3),0,0,A3, phi1)
    
    monomer_array_c2 = get_monomer_xyzR(monomer_name,0,0,0,0,0,-A3, phi1)##centerのc
    monomer_array_i02 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],0,0,-A3, phi1)
    monomer_array_ip5 = get_monomer_xyzR(monomer_name,c[0],c[1]+b_,c[2]+R3,0,0,-A3, phi1)
    monomer_array_ip6 = get_monomer_xyzR(monomer_name,c[0],c[1]-b_,c[2]-R3,0,0,-A3, phi1)
    monomer_array_ip7 = get_monomer_xyzR(monomer_name,c[0]+a_,c[1],c[2]+2*R4-R3,0,0,-A3, phi1)
    monomer_array_ip8 = get_monomer_xyzR(monomer_name,c[0]-a_,c[1],c[2]-(2*R4-R3),0,0,-A3, phi1)

    monomer_array_i01 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],0,0,A3, phi1)
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a_/2,b_/2,R4,0,0,-A3, phi2)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a_/2,-b_/2,R4-R3,0,0,-A3, phi2)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a_/2,-b_/2,-R4,0,0,-A3, phi2)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a_/2,b_/2,-R4+R3,0,0,-A3, phi2)
    monomer_array_it1 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]+b_/2,c[2]+R4,0,0,-A3, phi2)
    monomer_array_it2 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]-b_/2,c[2]+R4-R3,0,0,-A3, phi2)
    monomer_array_it3 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]-b_/2,c[2]-R4,0,0,-A3, phi2)
    monomer_array_it4 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]+b_/2,c[2]-R4+R3,0,0,-A3, phi2)
    ...
    dimer_array_p2 = np.concatenate([monomer_array_c1,monomer_array_p2])
    ...
    dimer_array_ip5 = np.concatenate([monomer_array_c2,monomer_array_ip5])
    dimer_array_ip6 = np.concatenate([monomer_array_c2,monomer_array_ip6])
    dimer_array_ip7 = np.concatenate([monomer_array_c2,monomer_array_ip7])
    dimer_array_ip8 = np.concatenate([monomer_array_c2,monomer_array_ip8])
    ...
    line_list_dimer_ip5 = get_xyzR_lines(dimer_array_ip5,file_description+'_ip5')
    line_list_dimer_ip6 = get_xyzR_lines(dimer_array_ip6,file_description+'_ip6')
    line_list_dimer_ip7 = get_xyzR_lines(dimer_array_ip7,file_description+'_ip7')
    line_list_dimer_ip8 = get_xyzR_lines(dimer_array_ip8,file_description+'_ip8')
