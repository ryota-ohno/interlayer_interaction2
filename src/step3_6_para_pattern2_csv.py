##層間相互作用の計算で2パターン考える tetracene1 6分子
import os
...
from make_6_para_pattern2 import exec_gjf
from step3_6_para_vdw_pattern2 import get_c_vec_vdw
from step3_6_para_vdw_pattern2 import detect_peaks
...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        df_params=pd.read_csv('~/Working/interlayer_interaction2/{}/assets/step2_result.csv'.format(monomer_name))
...
def listen(args):
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=10:##計算する層状の分子数
            ...
            E = (Ei01+Eip1+Eip2+Ei02+Eip3+Eip4)/2+Eit1+Eit2+Eit3+Eit4
            #### TODO
